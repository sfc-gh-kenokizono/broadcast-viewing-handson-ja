...
CREATE ROLE IF NOT EXISTS BCAST_ENGINEER_ROLE
  COMMENT = '生データから集計までを扱うロール。dbt の実行もこのロール';
CREATE ROLE IF NOT EXISTS BCAST_ANALYST_ROLE
  COMMENT = '集計済みの層だけを参照するロール';
...
GRANT ROLE BCAST_ANALYST_ROLE  TO USER IDENTIFIER($current_user_name);

-- カスタムロールは SYSADMIN の下にぶら下げる（Snowflake の推奨構成）
-- ここを省くと、BCAST_ENGINEER_ROLE が作ったテーブルに ACCOUNTADMIN でも到達できません。
...
-- Cortex の機能を使うための権限
GRANT DATABASE ROLE SNOWFLAKE.CORTEX_USER TO ROLE BCAST_ENGINEER_ROLE;
GRANT DATABASE ROLE SNOWFLAKE.CORTEX_USER TO ROLE BCAST_ANALYST_ROLE;
...
-- 権限の付与
-- -----------------------------------------------------------------------------
GRANT USAGE ON WAREHOUSE BCAST_HANDSON_WH TO ROLE BCAST_ENGINEER_ROLE;
GRANT USAGE ON WAREHOUSE BCAST_HANDSON_WH TO ROLE BCAST_ANALYST_ROLE;

GRANT USAGE ON DATABASE BCAST_VIEWING_HANDSON TO ROLE BCAST_ENGINEER_ROLE;
GRANT USAGE ON DATABASE BCAST_VIEWING_HANDSON TO ROLE BCAST_ANALYST_ROLE;

-- エンジニアは 4 つの層すべてを扱える
GRANT ALL ON SCHEMA BCAST_VIEWING_HANDSON.RAW  TO ROLE BCAST_ENGINEER_ROLE;
GRANT ALL ON SCHEMA BCAST_VIEWING_HANDSON.STG  TO ROLE BCAST_ENGINEER_ROLE;
GRANT ALL ON SCHEMA BCAST_VIEWING_HANDSON.INT  TO ROLE BCAST_ENGINEER_ROLE;
GRANT ALL ON SCHEMA BCAST_VIEWING_HANDSON.MART TO ROLE BCAST_ENGINEER_ROLE;
GRANT SELECT ON ALL TABLES IN SCHEMA BCAST_VIEWING_HANDSON.RAW TO ROLE BCAST_ENGINEER_ROLE;
GRANT SELECT ON FUTURE TABLES IN SCHEMA BCAST_VIEWING_HANDSON.RAW TO ROLE BCAST_ENGINEER_ROLE;

-- アナリストは集計済みの層だけ。RAW / STG / INT には触れられない
GRANT USAGE ON SCHEMA BCAST_VIEWING_HANDSON.MART TO ROLE BCAST_ANALYST_ROLE;
GRANT SELECT ON FUTURE TABLES IN SCHEMA BCAST_VIEWING_HANDSON.MART TO ROLE BCAST_ANALYST_ROLE;
GRANT SELECT ON FUTURE VIEWS  IN SCHEMA BCAST_VIEWING_HANDSON.MART TO ROLE BCAST_ANALYST_ROLE;
...
GRANT USAGE ON INTEGRATION BCAST_GIT_API TO ROLE BCAST_ENGINEER_ROLE;
GRANT USAGE ON SCHEMA BCAST_VIEWING_HANDSON.INTEGRATIONS TO ROLE BCAST_ENGINEER_ROLE;

-- 公開リポジトリなので GIT_CREDENTIALS を指定しません
CREATE OR REPLACE GIT REPOSITORY BCAST_VIEWING_HANDSON.INTEGRATIONS.BCAST_REPO
  API_INTEGRATION = BCAST_GIT_API
  ORIGIN = 'https://github.com/sfc-gh-kenokizono/broadcast-viewing-handson-ja.git';

GRANT READ ON GIT REPOSITORY BCAST_VIEWING_HANDSON.INTEGRATIONS.BCAST_REPO
...
CREATE OR REPLACE STAGE BCAST_VIEWING_HANDSON.INTEGRATIONS.BCAST_RAW_STAGE
  COMMENT = 'リポジトリから取り込んだ CSV の置き場所';

COPY FILES
  INTO @BCAST_VIEWING_HANDSON.INTEGRATIONS.BCAST_RAW_STAGE
  FROM @BCAST_VIEWING_HANDSON.INTEGRATIONS.BCAST_REPO/branches/main/data/;

GRANT READ ON STAGE BCAST_VIEWING_HANDSON.INTEGRATIONS.BCAST_RAW_STAGE
  TO ROLE BCAST_ENGINEER_ROLE;

-- 11 ファイルが並んでいれば成功です
LS @BCAST_VIEWING_HANDSON.INTEGRATIONS.BCAST_RAW_STAGE;
...
-- ログがない時間は TV を見ていない時間です。1 台の受信機は同時に 1 局だけを
-- 見るため、正常なデータでは同じ COMMON_ID の視聴区間は重なりません。
...
CREATE OR REPLACE TABLE CM_MASTER (
  ...
  IS_ANALYSIS_TARGET BOOLEAN COMMENT 'ハンズオンで接触を分析する 20 素材なら TRUE'
) COMMENT = '局全体のコマーシャル素材マスタ。100 素材のうち 20 素材を分析対象にする';
...
) COMMENT = '局全体のコマーシャル放送実績。1 行が 1 素材の 1 回の放送';
...
--
--   視聴が 24 時間を超える
--     視聴終了の記録に失敗し、実際より後の時刻が VIEW_TO に入った可能性がある行です。
--     「24 時間以上ずっと視聴した」と判断せず、終了時刻の記録異常を疑います。
...
-- 次のようになります。
--   全行数 1,050,648 / 終了が開始より前 200 / 24 時間超 148 / 重複 300
--
-- この 648 行を除いた 1,050,000 行が、第 2 章のクレンジング後の行数になります。
...
--   ENABLED = TRUE;
--
-- GRANT READ ON SECRET BCAST_GIT_SECRET TO ROLE BCAST_ENGINEER_ROLE;
--
