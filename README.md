...
非特定視聴データを題材に、データの取り込みからデータ変換、セマンティックレイヤー、AI エージェント、可視化アプリまでを Snowflake の中だけで一本通すハンズオン教材です。所要時間は約 2 時間 40 分で、Snowsight のブラウザ画面だけで完結します。ローカルへのツールのインストールやコマンドライン操作は必要ありません。
...
- 1 台の TV は同時に 1 局だけを視聴し、ログのない時間は TV を見ていない時間として扱う
- 60 秒ごとの信号を視聴区間（from-to）に畳む処理と、区間を 1 分単位に展開する処理を、両方向とも手で確かめる
- 3 階層の ID（各局 ID、共通 ID、IP アドレス）を名寄せし、同じデータから粒度の違う数字を出し分ける
- dbt でクレンジングから指標算出までのパイプラインを組み、依存関係を図で確認する
- 視聴区間（from-to）のデータを 1 分単位に展開したときに、データ量がどれだけ増えるかを実測し、展開したものを集計して小さく持つ
- リーチ、フリークエンシー、インプレッションの関係を、同じ定義から出す
- 指標の定義を Snowflake 側に持たせ、自然言語で分析できる状態にする
- 番組やコマーシャルの説明文を検索できるようにし、数値の集計と組み合わせて回答させる
- マート層をそのまま可視化アプリにし、毎分の視聴台数カーブも描く
- 生データと集計データでアクセス権を分け、識別子に伏せ字をかける
- （任意・2 人で）集計データをコピーせずに別アカウントへデータシェアする
...
| 1b | Snowflake そのものの動きを体感する（補足） | 15 分 | [docs/step1b_basics.md](docs/step1b_basics.md) | [sql/step1b_basics.sql](sql/step1b_basics.sql) |
...
| コマーシャルの放送実績（局全体、1 行 = 1 回の放送） | 303,600 |
| 番組の放送枠（各局 4:00〜24:00） | 8,747 |
| 番組マスタ / CM素材マスタ / 局マスタ | 60 / 100 / 5 |
| ハンズオンで接触を分析するCM素材 | 20 |
| パネル調査で属性が判明している端末 | 2,000（全体の 10 パーセント） |

対象期間は 2026 年 5 月 1 日から 7 月 31 日までの 92 日間、3 ヶ月分です。月をまたいで積み上がるデータを見てもらうため、視聴は期間の後半にかけてゆるやかに減るようにしてあります。各局は毎日 4:00〜24:00 の 20 時間を放送し、およそ 20 分ごとに 3 分の CM ブレークがあります。CM素材100件のうち、ハンズオンで接触を分析する20件には **2週間から4週間の出稿期間**があります。すべて架空のデータで、実在の放送局や番組、広告主とは関係ありません。

視聴ログは、受信機が送る 60 秒ごとの信号を局側で連続視聴区間（from-to）に畳んだあとの形を出発点にしています。畳む処理自体は第 1b 章の補足で小さなサンプルを使って確かめます。
...
この教材では、名寄せが済んでいる前提のデータを作って見せることはしません。代わりに、**5 局の放送データを集めたことで初めて出る数字**に絞っています。視聴終了から30分以内に次に見た別の局がその代表例で、これは1局のデータだけでは把握できません。
...
[sql/cleanup.sql](sql/cleanup.sql) を実行すると、このハンズオン名で作ったデータベース、ウェアハウス、ロールなどを削除します。ワークスペースは画面から削除し、アカウント共通のCortexクロスリージョン設定は必要に応じて元へ戻します。
