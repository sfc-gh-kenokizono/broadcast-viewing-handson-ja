...
ワークスペースの左側の一覧から `sql` フォルダを開き、このファイルを選んで上から順に実行します。

この章は、途中で 1 回画面操作が入ります（第 4 章のエージェントも画面で作ります）。セマンティックビューは SQL で作り、**検索サービスは Snowsight の画面で作ります**。画面で作ってみて、どんな選択肢があるかを目で見てもらうためです。間に合わなかった人のために、ファイルの末尾に同じものを SQL で作る保険を置いてあります。

```text
sql/step3a_semantic.sql
  1. セマンティックビュー          ← SQL を実行
  2. 説明文をまとめたビュー          ← SQL を実行
     【画面】検索サービスを作る     ← Snowsight の AI と ML → Cortex Search
  3. 権限を渡す                      ← SQL を実行
  4. 動作確認                          ← SQL を実行
  保険：SQL で一気に作成              ← 画面が間に合わないときだけ
```
...
視聴ログは数値と ID だけなので、文章として探せる材料が別に必要です。番組の概要文とコマーシャルの素材説明文をひとつのビュー `V_PROGRAM_CM_DOCS` にまとめ（ここまでは SQL）、それを検索の対象にします。

本文 `CONTENT` には番組名やジャンルも含めています。概要文だけを対象にすると「アニメ」のような語で当たりにくくなるためです。ビューは番組 60 件＋コマーシャル 20 件の 80 行です。

### 検索サービスを画面で作る

スクリプトの 2 節で `V_PROGRAM_CM_DOCS` を作ったあと、Snowsight の画面に移ります。

```text
Snowsight → 左メニュー AI と ML → Cortex Search
 → 右上の 作成（Create）
```

画面右上のロールが `ACCOUNTADMIN` になっていることを確認してから進めます。

| 項目 | 値 |
|---|---|
| サービスのデータベースとスキーマ | `BCAST_VIEWING_HANDSON` / `MART` |
| サービス名 | `SVC_PROGRAM_CM_META` |
| インデックスを作成するデータを選択 | `BCAST_VIEWING_HANDSON.MART.V_PROGRAM_CM_DOCS` |
| 検索列 | `CONTENT` |
| 属性列 | `DOC_TYPE`, `DOC_ID`, `TITLE`, `GENRE`, `TIME_SLOT`, `NETWORK_NAME`, `ADVERTISER`, `CATEGORY` |
| サービスに含む列を選択 | すべて選択 |
| ターゲットラグ | 1 時間 |
| 埋め込みモデル | 既定のまま |
| インデックス作成用のウェアハウス | `BCAST_HANDSON_WH` |

→ **作成** をクリック

| 画面の項目 | 意味 |
|---|---|
| 検索列 | 検索の対象になる本文の列。SQL では `ON` |
| 属性列 | 絞り込みに使える列。SQL では `ATTRIBUTES` |
| ターゲットラグ | 元のデータが変わったときに、どれだけの遅れまで許すか |

作成直後は索引を作っている途中です。80 行なので数十秒で終わります。ワークスペースに戻って、スクリプトの `SHOW CORTEX SEARCH SERVICES` で 1 行返れば成功です。そのあと 3 節（権限）に進みます。

### 画面が間に合わないとき

ファイルの末尾に「保険：UI を使わず SQL で一気に作成する場合はこちら」があります。`/* */` で囲んであるので、中の SQL だけを選んで実行します。画面で入力する値と完全に同じ内容です。
