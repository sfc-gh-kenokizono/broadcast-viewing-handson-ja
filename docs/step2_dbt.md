...
1. 左のファイル一覧で `dbt` を開き、`models` → `staging` → `stg_viewing_log.sql` を選びます
2. エディタにモデルの SQL が表示されます
3. **エディタの上のバー**に、dbt の操作部分が現れます

| 選ぶもの | 選ぶ値 |
|---|---|
| Project | `bcast_viewing`（`dbt` フォルダのプロジェクト。1 つしかないので既定で選ばれています） |
| Profile | `dev` |
| コマンド | ここから `Compile` や `Run` を選びます |

4. **エディタの下のバー**で `dbt` を選ぶと、実行結果を見るパネル（Output / DAG）が開きます

`dbt deps` は不要です。このプロジェクトは外部パッケージを使っていないので、そのまま次へ進めます。

## 4. コンパイルして依存関係を見る

1. コマンドの一覧から `Compile` を選び、実行ボタンを押します
2. 下のパネルで `Output` タブを開くと、Snowflake 上で実行された命令と dbt の出力が見えます
3. `DAG` タブを開きます
...
`Compile` はテーブルを作りません。SQL を組み立てて依存関係を確認するだけです。
...
> **どのロールで動くのか**
>
> `Run` は `profiles.yml` に書いたロールで実行されます。このプロジェクトでは `BCAST_ENGINEER` です。画面右上で選んでいるロールではありません。
>
> 第 1 章で自分自身に `BCAST_ENGINEER` を付与しているので、そのまま動きます。付与していないと権限エラーになります。

### モデル 1 つだけを実行する

モデルのファイルを開いた状態で再生ボタンを押すと、そのモデルだけが実行されます（`dbt run --select <モデル名>` に相当）。ふつうの SQL ファイルとして実行されるのではありません。1 つのモデルを直しながら試すときに使います。
