...
2 つめが dbt を使う最大の理由です。10 個のモデルの実行順序を人が管理する必要がありません。`ref()` と `source()` で「何を参照しているか」を書いておけば、**順序は dbt が決めます**。
...
```
Found 10 models, 21 data tests, 11 sources
Completed successfully
```

**10 モデル・21 テスト・11 ソース**が認識されていれば成功です。ここで数が合わないなら、ファイルが欠けているか、置き場所が違います。
...
RAW 11 テーブル
   ...
   │                                              │
   │                                              └──→ int_viewing_minutes ────→ mart_minute_audience
```
...
これは「**このモデルを直すと、下流の 6 個に影響する**」という意味です。クレンジングの条件をひとつ変えるだけで、リーチもフリークエンシーもチャンネル遷移も全部変わります。影響範囲がこうして見えるのが DAG の一番の使いどころです。

**③ `int_viewing_minutes` の下流が `mart_minute_audience` だけであること**

1 分単位に展開したテーブル（約 1,208 万行）を使っているのは、毎分の視聴台数を数える `mart_minute_audience` だけです。リーチもフリークエンシーも CM 接触も、展開しない from-to のまま算出しています。

```text
📦 from-to（105 万行） ───────────────────────────────────────────▶ リーチ・フリークエンシー・CM 接触・番組・遷移
        │
        └─展開─▶ 📂 1 分ごと（1,208 万行） ──縮約─▶ 📊 局 × 分 × 台数（約 55 万行） ─▶ 毎分カーブ
```

ここが「保存は from-to、展開は必要な指標だけ、展開したものは集計して小さく持つ」という使い分けを図で確認できる場所です。第 10 節で行数を実際に数えます。

実務では、DAG の右端が行き止まりになっているモデルは**確認すべき合図**です。大きなテーブルを毎回作っていて誰も使っていないなら、それは払う必要のないコストです。**DAG は使われていないモデルを見つける道具にもなります。**
...
dbt は 10 個のモデルをまとめて作ります。待つだけの時間を短くするため、ここでは
...
10 個のモデルが依存関係の順に作られます。処理中は出力タブを眺めながら待ちます。
...
`STG` と `INT` は基本的にビューです。参照されたときに計算します。`MART` は可視化アプリやエージェントが繰り返し参照するので、計算結果を持たせています。`int_viewing_minutes` だけ `INT` でもテーブルにしているのは、約 1,208 万行の展開を毎回やり直すのが無駄だからです。
...
| `mart_minute_audience` | 1 局 × 1 分（台数は集計済み） |
...
`mart_minute_audience` だけは例外で、台数を集計した形で持っています。毎分のカーブを描くには「何台」だけあれば十分で、「誰が」を持ち続けると 1,208 万行のままになるためです。
...
全部で **21 件**です。20 件の宣言テストに加え、「同じTVの視聴区間が重ならない」ことを確かめるSQLテストが1件あります。出力タブに次のように出ます。

```
Finished running 21 data tests
Completed successfully
Done. PASS=21 WARN=0 ERROR=0 SKIP=0
```

**`PASS=21 ERROR=0`** が正解です。
...
SELECT
  ...
  (SELECT COUNT(DISTINCT IP_ADDRESS)  FROM BCAST_VIEWING_HANDSON.MART.MART_DEVICE_DAILY) AS "IPアドレスの数";
```
...
| IP アドレスで数えたIP数 | 世帯到達の代理指標。1つのIPを複数台が共有するため、台数より少ない |
...
「041の視聴後、30分以内に071を見た」のような、局をまたいだ次の視聴が出てきます。間隔0分は直接のチャンネル変更、正の間隔はTVを見ていない時間を挟んでいます。
...
## 10. 1 分単位に展開すると行数がどうなるか

同じ視聴を表す形は 3 つあります。中身の情報はどれも同じで、違うのは行数だけです。

```text
📡 60 秒ごとの信号（受信機が局に送る形）      12 行   ← 第 1b 章の補足で畳んだ
📦 from-to（局が畳んで基盤に渡す形）            1 行   ← RAW / STG はこれ
📂 1 分ごとの行（INT で展開した形）            12 行   ← int_viewing_minutes
```
...
```sql
SELECT
  (SELECT COUNT(*) FROM BCAST_VIEWING_HANDSON.STG.STG_VIEWING_LOG)     AS "展開前（from-to）",
  (SELECT COUNT(*) FROM BCAST_VIEWING_HANDSON.INT.INT_VIEWING_MINUTES) AS "展開後（1 分ごと）",
  (SELECT COUNT(*) FROM BCAST_VIEWING_HANDSON.MART.MART_MINUTE_AUDIENCE) AS "縮約後（局 × 分）",
  ROUND(
...
```
展開前（from-to）      1,050,000
展開後（1 分ごと）    12,075,722
縮約後（局 × 分）          548,610
平均の視聴区間長          11.4 分
```

展開後の行数は、展開前の行数に平均の視聴区間長を掛けた値とほぼ一致します。**平均が M 分なら行数はおよそ M 倍**という関係です。平均が 30 分なら 30 倍、60 分なら 60 倍になります。

### いつ展開するのか

ふだんは from-to のままで分析します。リーチもフリークエンシーも CM 接触も番組視聴もチャンネル遷移も、この章のマートはすべて from-to から直接作っています。「この区間に CM の時刻が入っているか」「区間と番組枠が重なっているか」という**重なり判定**で答えられるからです。

展開したくなるのは、**時間軸を細かく刻んでグラフにするとき**だけです。「20 時台の毎分の視聴台数カーブを描きたい」なら、分の行があれば `GROUP BY 分` で終わります。区間のままだと「20:00 を含む区間は？ 20:01 を含む区間は？…」を 60 回問うことになります。

### 展開したものは集計して小さく持つ

展開した 1,208 万行を提供層にそのまま置くと、保存量も参照時の計算量も M 倍のまま残ります。毎分のカーブに必要なのは「何台」だけなので、`mart_minute_audience` では「誰が」を捨てて「局 × 分 × 台数」に集計しています。約 55 万行です。
...
3. **保存は from-to、展開は必要な指標だけ、展開したものは集計して小さく持つ**。全件を常に分の形で持ち続けないことが基本です

見積もりを立てるときは、生データの量ではなく、**どの指標のためにどこまで粒度を上げるか**が効いてきます。第 5 章のアプリで、このマートから毎分のカーブを実際に描きます。
...
SELECT
  ...
  COUNT(DISTINCT IP_ADDRESS)                AS "視聴IP数",
...
推計視聴人数は、視聴IP数に1回線あたりの想定人数2.2を掛けた参考値です。IP数は世帯数そのものではないため、正確な人数ではありません。実際にはパネル調査などを正解データにして受信機ごとに人数と属性を割り当てます。
