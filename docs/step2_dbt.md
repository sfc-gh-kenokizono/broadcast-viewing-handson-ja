...
```
Found 9 models, 17 data tests, 11 sources
Completed successfully
```

**9 モデル・17 テスト・11 ソース**が認識されていれば成功です。ここで数が合わないなら、ファイルが欠けているか、置き場所が違います。
...
ただし実務では、この「行き止まり」は**確認すべき合図**です。12,075,722 行のテーブルを毎回作っていて、誰も使っていないなら、それは払う必要のないコストです。**DAG は使われていないモデルを見つける道具にもなります。** お客様の環境で最初に見る観点として使えます。
