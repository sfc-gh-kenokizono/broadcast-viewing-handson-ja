...
[sql/step4_agent.sql](../sql/step4_agent.sql)
...
データの読み方を間違えないようにする決まりを、ここに書いておきます。このハンズオンでは次の 3 つを入れています。
...
```
配信を足すとリーチはどれだけ広がりましたか。広告主ごとに教えてください
```

これが増分リーチです。放送と配信のデータが 1 か所にそろっていないと計算できない指標が、自然言語で出てきます。
